...
class IntegrationSpec extends PlaySpecification {
  ...
  "Application" should {
    ...
    "Get 'wallet not found' response with unknown wallet" in new WithApplication {
      val wallet = route(app, FakeRequest(GET, "/wallet/0")).get
    ...
    "Get the right wallet with the right id" in new WithApplication {
      val wallet = route(app, FakeRequest(GET, "/wallet/1")).get
      status(wallet) must equalTo(OK)
      val walletJson = contentAsJson(wallet)
    ...
    "Failure to make purchase with wrong wallet id" in new WithApplication {
      val purchaseStatus = route(app, FakeRequest.apply(POST, "/products/3/purchase", FakeHeaders(Seq(HeaderNames.HOST -> "localhost")), Json.toJson(User(0)))).get
      status(purchaseStatus) must equalTo(OK)
    ...
    "Get some flash sales data" in new WithApplication {
      val sales = route(app, FakeRequest(GET, "/sales/current?country=SG")).get
      status(sales) must equalTo(OK)
      val salesJson = contentAsJson(sales)
      (salesJson \ "sales").as[Seq[ProductSale]].size > 0 must beTrue
    }
  }
}
